<template>
    <section class="space-y-6">
        <div class="flex justify-between items-center mb-8 pb-4 border-b border-slate-200">
            <h2 class="text-3xl font-extrabold text-sky-800">Family Suggestions</h2>
            <div class="flex items-center space-x-4">
                <button
                    @click="pick('boy')"
                    class="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-lg shadow-md transition-all duration-200
                       flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-sm"
                ...
                <button
                    @click="pick('girl')"
                    class="px-5 py-2 bg-pink-600 hover:bg-pink-700 text-white font-semibold rounded-lg shadow-md transition-all duration-200
                       flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-sm"
                    :disabled="props.suggestions.length === 0"
                >
                    <SparklesIcon class="w-5 h-5" />
        ...
        >
            ...
            >
                <div>
                    <p class="font-semibold text-lg text-slate-800 flex items-center gap-2">
                        <UserIcon class="w-5 h-5 text-sky-600" />
                        {{ s.family_member_name }}
                        <span class="text-slate-400 font-normal text-sm ml-1">({{ s.relation || 'N/A' }})</span>
                    </p>
                    <div class="text-slate-600 text-sm mt-1 flex space-x-4">
                        ...
                        <div class="flex items-center gap-1">
                            <HeartIcon class="w-4 h-4 text-pink-500 fill-pink-500" />
        ...
        <p v-else class="text-center text-slate-500 border border-slate-200 rounded-xl py-12 bg-white shadow-sm italic">
        ...
        >
            <h3 class="text-3xl font-extrabold text-sky-700 mb-2 flex items-center justify-center gap-3">
                <GiftIcon class="w-7 h-7 text-sky-500" />
                Picked Name Reveal
            </h3>
            <p class="text-xl text-slate-800 mt-4">
                The lucky name is <span class="font-extrabold text-sky-600">{{ picked.selected_name }}</span>!
            </p>
...
import { SparklesIcon, GiftIcon, XMarkIcon, FaceSmileIcon, HeartIcon, UserIcon} from "@heroicons/vue/24/solid";
